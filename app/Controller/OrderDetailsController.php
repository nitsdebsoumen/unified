 ...
 */
	...
	var $uses = array('OrderDetail','Order','Shop','Country','User','Category','Attribute','AttributeItem','UserPaymentDetail','UserBillingAddress','UserCreditCard','Listing','ListAttribute','ListAttributeItem','ListTag','ListMaterial','ListDispatch','ListImage','ListFile','ShopSetting','ShopFollowing','ShippingAddress','SiteSetting');
 ...
 */
	public function add() {
		...
		$users = $this->OrderDetail->User->find('list');
		...
		if ($this->request->is(array('post', 'put'))) {
			...
			if ($this->OrderDetail->save($this->request->data)) {
				...
				/*******/
					...
					$options = array('conditions' => array('Listing.id' => $orderdetail['OrderDetail']['list_id']));
					$listItem = $this->Listing->find('first', $options);

					#$link = 'http://192.232.197.138/shopfit/order_details/index/'.base64_encode($orderId);
					$msg_body = 'Hi '.$orderBy['User']['first_name'].'<br/><br/>Your ordered product '.$listItem['Listing']['item_tittle'].' has been Shipped. Please contact ShopsFit at '.$adminEmail.' in case of not receiving your product. Hope you liked the ShopsFit experience.<br/><br/>Thanks,<br/>ShopsFit';
					...
					$Email->subject('ShopsFit Items Shipped From Shop');
					$Email->send($msg_body);
					/*******/
				}
