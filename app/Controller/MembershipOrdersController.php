 ...
 */
	...
	public function admin_index() {
		...
		$this->MembershipOrder->recursive = 0;
		$MembershipOrders = $this->Paginator->paginate('MembershipOrder');
		//pr($MembershipOrders); exit;
		$this->set(compact('MembershipOrders'));
	}
 ...
 */
	...
	public function ajaxFeatureProvider(){
		$data = array();
		if(isset($this->request->data)){
			...
			if($user_details['User']['featured'] == 0){
				$this->request->data['User']['featured'] = 1;
				$this->User->id = $user_id ; 
				if($this->User->save($this->request->data)){
					$data['ack'] = 1;
					$data['res'] = 'User successfully featured';
				}

			}
			else{
				$data['ack'] = 0;
				$data['res'] = 'User already featured';
			}
		}
