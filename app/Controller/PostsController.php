...
class PostsController extends AppController {
    ...
    public function admin_add($id = NULL) {
        ...
        $users = $this->User->find('all',array('conditions' => array('User.is_admin' => '0','User.admin_type'=>'2') ));
        ...
        $company_list = $this->CompanyDetail->find('list', array('fields' => array('CompanyDetail.user_id', 'CompanyDetail.company_name')));
    ...
    if($user_id!=''){
      ...
      $this->set('user_company',$user_company);  
        ...
        $this->set(compact('title_for_layout', 'countries', 'posts', 'users', 'categories', 'skills','company_list'));
    }
    ...
    public function add_course() {
        ...
        $locations = $this->Post->CourseLocation->find('list', array('fields' => array('CourseLocation.id', 'CourseLocation.address'),'conditions' => array('CourseLocation.user_id' => $userid)));
        ...
        if ($this->request->is('post')) {
            ...
            //if (isset($this->request->data['Post']['image']) && $this->request->data['Post']['image']['name'] != '') {
                ...
                // $user_locations=$this->request->data['Post']['location'];

                // $user_location_count=count($user_locations);

                $this->request->data['Post']['user_id'] = $userid;
                $this->request->data['Post']['location']='';
                $this->Post->create();
                if ($this->Post->save($this->request->data)) {
                    $this->request->data1['PostImage']['post_id'] = $this->Post->id;
                    $this->PostImage->save($this->request->data1);
                   ...
                   //  $inserted_id=$this->Post->getInsertID();
                   //  $this->loadModel('UserCourseLocation');
                   //  for($i=0;$i<=($user_location_count-1);$i++)
                   //  {
                   //      $this->data2['UserCourseLocation']['post_id'] = $inserted_id;
                   //      $this->data2['UserCourseLocation']['course_locations_id'] = $user_locations[$i];

                   //      $this->UserCourseLocation->create();
                   //      $this->UserCourseLocation->save($this->data2);

                   //      print_r($UserCourseLocation);
                   //  }
        ...
        $skills = $this->Post->Skill->find('list',array('fields'=>array('id','skill_name')));
        $this->set(compact('title_for_layout', 'countries', 'posts', 'users', 'categories', 'skills','locations'));
    }
    ...
    public function admin_edit($id = null) {
        ...
        $this->set(compact('posts', 'users', 'categories', 'skills', 'countries', 'states', 'cities', 'location'));
    }
