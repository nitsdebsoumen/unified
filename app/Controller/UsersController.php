...
//                }
    ...
    public function user_logo_upload($upload,$image){
        ...
        if (isset($upload['name']) && $upload['name'] != '') {
            ...
            if ($ext) 
            {
                $uploadPath = Configure::read('UPLOAD_USER_LOGO');
                $extensionValid = array('jpg', 'jpeg', 'png', 'gif');
                if (in_array($ext, $extensionValid)) {
                    $OldImg = $image;
                    $imageName = rand() . '_' . (strtolower(trim($upload['name'])));
                    $full_image_path = $uploadPath . '/' . $imageName;
                    move_uploaded_file($upload['tmp_name'], $full_image_path);
    ...
    public function editprofile() {
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            if (!empty($emailExists)) {
                $this->Session->setFlash('Email already exists.', 'default', array('class' => 'error'));
                        ...
                        if (isset($this->request->data['User']['user_logo']['name']) && $this->request->data['User']['user_logo']['name'] != '') {   
                             $newImage = $this->user_logo_upload($this->request->data['User']['user_logo'],$this->request->data['User']['image']);
                             if($newImage!='')
                             {
                                $this->request->data['User']['user_logo'] = $newImage;
                             } 
                        }   
                        else 
                        {
                            unset($this->request->data['User']['user_logo']);
                        }
        ...
        $lgas = $this->User->Lga->find('list', array('fields' => array('Lga.id', 'Lga.local_name'),'order' => array(
          'Lga.local_name' => 'asc'
        )));
        //pr($states);
        
        $this->set(compact('countries', 'states', 'cities','userDetails','lgas'));
...
//        exit();
        ...
        $skills = $this->Post->find('all',array('limit'=>8,'order'=>array('Post.click_count'=>'DESC')));

        $this->Comment->recursive = 2;
        $comments = $this->Rating->find('all',array('limit'=>3,'order' => array('Rating.comfort' => 'DESC')));
            ...
            if (!empty($courses)) {
                ...
                $html .= '<div class="media-left media-middle">'.
                        ...
                        '<div class="media-body">'.
                            ...
                            '<p>'.substr(strip_tags($course['Post']['post_description']), 0, 100).'</p>'.
                            '<ul>'.
                                '<li><i class="fa fa-user"></i> <p>'.$course['Post']['quantity'].' Course available</p></li>'.
                                '<li>'.
                        ...
                        '<div class="media-right media-middle">'.
                            ...
                            '<button class="more_info" onclick="location.href=\''. $this->webroot . 'users/coursedetail/'.$course['Post']['slug'].'\'">More Info</button>'.
                        '</div>'.
                    '</div>';
                }
    ...
    public function coursedetail($slug = NULL) {
        ...
        if (isset($userid) && $userid != '') {
            $options = array('conditions' => array('Order.user_id'=> $userid,'Order.post_id'=>$id));
            $this->loadModel('Order');
            $orders = $this->Order->find('count', $options);
            if($orders>0){
                $review_status = 1;
            }
            else{
        ...
        $this->set(compact('review_status'));
  ...
  public function ajaxremoveWishlist(){
    ...
    public function cms_page($slug = NULL)
    {
        $this->loadModel('CmsPage');
        $content = $this->CmsPage->find('first',array('conditions'=>array('CmsPage.slug' => $slug)));
    ...
    public function membership_checkout($id = NULL){
        ...
        if(isset($userid)){
            ...
            $setting = $this->Setting->find('first',array('conditions'=>array('Setting.id'=>1)));
            $this->loadModel('MembershipOrder');
            $user_stat = $this->MembershipOrder->find('count',array('conditions'=>array('MembershipOrder.user_id'=>$userid)));
            if($user_stat>0){ $user_order = 0; }else{ $user_order = 1; }
            $this->set(compact('plan','user','setting','user_order'));        
        }
        else{
            return $this->redirect(array('action' => 'login'));
        }
    ...
    public function membership_payment_success(){
         ...
         $this->request->data['user_id']            = $this->Session->read('userid');;
         $this->request->data['membership_plan_id'] = $this->request->data['plan_id'];
         $this->request->data['transaction_id']     = $this->request->data['reference_id'];
         $this->request->data['amount']             = $this->request->data['amount'];
         $this->request->data['billing_address']    = $this->request->data['billing_address'];
         $this->request->data['shipping_address']   = $this->request->data['shipping_address'];
         $returned_transref = $this->request->data['reference_id'];
         $this->loadModel('MembershipOrder');
         $this->loadModel('MembershipPlan');
         $tid=$this->MembershipOrder->find('all',array('conditions'=>array('MembershipOrder.transaction_id'=>$returned_transref),'fields'=>array('MembershipOrder.transaction_id')));
         $data=$this->MembershipPlan->find('first',array('coditions'=>array('MembershipPlan.id'=>$this->request->data['transaction_id'])));

         if(empty($tid)){
            $this->User->id = $this->request->data['user_id'];
            $this->User->saveField('membership_plan_id',$this->request->data['membership_plan_id']);
            $this->MembershipOrder->save($this->request->data);
            $status='Order of Membership is Placed Successfully';
            $data['Ack'] = 1;
            $data['res'] = $status;
    ...
    public function user_dashboard(){
        ...
        if(isset($userid)){
            ...
            //$courses = $this->Post->find('all',array('conditions'=>array('Post.user_id'=>$id)));
            //$venues = $this->Venue->find('all',array('conditions'=>array('Venue.user_id'=>$id)));
            $course_orders = $this->OrderItem->find('all',array('conditions'=>array('OrderItem.user_id'=>$id)));
            $click_count = $this->Post->find('all',array('conditions'=>array('Post.user_id'=>$id),'fields' => array('sum(Post.click_count)   AS click_count')));
            $top_courses = $this->Post->find('all',array('conditions'=>array('Post.user_id'=>$id),'order'=>array('Post.click_count DESC'),'limit'=>5));
            $this->loadModel('Rating');
            $total_no_of_reviews = $this->Rating->find('count',array('conditions'=>array('Rating.uid'=>$id))); 
            //$total_no_of_course = count($courses);
            //$total_no_of_venue  = count($venues);
            $total_course_order = count($course_orders);
            $total_course_view  = $click_count['0']['0']['click_count'];
            $this->loadModel('OrderItem');
            $this->Rating->recursive = 2;
            $ratings = $this->Rating->find('all',array('conditions'=>array('Rating.uid'=>$id),'limit'=>5));
            // pr($ratings); exit;
            
            $featured_courses = $this->Post->find('all',array('limit'=>8,'OrderItem'=>array('Post.click_count'=>'DESC')));
             $this->set(compact('featured_courses','total_no_of_course','total_no_of_reviews','total_course_order','total_course_view','top_courses','ratings'));
        }
        else{
            return $this->redirect(array('action' => 'login'));
        }
    ...
    public function edit_company_details(){
            ...
            if ($this->request->is(array('post', 'put'))) {
                ...
                if (isset($this->request->data['CompanyDetail']['logo']['name']) && $this->request->data['CompanyDetail']['logo']['name'] != '') {
                    ...
                    if ($ext) {
                        ...
                        if (in_array($ext, $extensionValid)) {
                            ...
                            $this->Session->setFlash(__('Invalid Image Type.'));
                            return $this->redirect('/users/provider_dashboard/edit_company_details','default', array('class' => 'error'));
                        }
                    }
                } else {
                ...
                if(!empty($this->request->data['CompanyDetail']['id'])){
                        if ($this->CompanyDetail->save($this->request->data)) {
                            $this->Session->setFlash('The Company Detail has been saved.','default', array('class' => 'success'));
                            return $this->redirect('/users/provider_dashboard/<?php echo base64_encode($userid);?>');
                        } else {
                            $this->Session->setFlash('The Company Detail could not be saved. Please, try again.','default', array('class' => 'error'));
                        }
                }
                else
                {
                    $this->CompanyDetail->create();
                        if ($this->CompanyDetail->save($this->request->data)) {
                            ...
                            $this->Session->setFlash(__('The Company Detail could not be saved. Please, try again.','default', array('class' => 'error')));
                        }
                }
        ...
        $banks = $this->CompanyDetail->Bank->find('list', array('fields' => array('Bank.id', 'Bank.bank_name')));
        ...
        // pr($states); exit;
        $this->set(compact('countries','states','cities','CompanyDetail','userid','lgas','banks'));

    }

    public function venue_page($slug = NULL){
        //$venue_id = base64_decode($id);
        //$this->Venue->recursive = 2;
        $venueDetails = $this->Venue->find('first',array('conditions'=>array('Venue.slug'=>$slug)));
        $this->set(compact('venueDetails'));
    }
    ...
    public function linkedinLoginRegister() {
        $data = $arr = array();
            ...
            $linkedin_check = $this->User->find('first', $linkedin_option);
                ...
                if ($linkedin_check['User']['activated'] == 0 || $linkedin_check['User']['activated']=='') {
                  ...
                  $emailExistsOption = array('conditions' => array('User.email_address' =>$ln_email));
                  $emailExists = $this->User->find('first', $emailExistsOption);
                if (empty($emailExists)) {
